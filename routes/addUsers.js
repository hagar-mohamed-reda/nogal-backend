...
route.post('/', check, (req, res) => {
    db.query(`select * from users where name = '${req.body.name}' or email = '${req.body.email}'`, (err00, row00, result00) => {
            if (row00.length > 0) {
                res.json({ check: false });
            } else {
                DB.table('users').create(req.body, () => {
                    // store log file for the user
                    log(req.session.user, " added a user Named " + req.body.name);
                    res.json({ check: true });
                });
            }
