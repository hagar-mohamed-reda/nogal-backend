...
route.post('/', check, upload.single('excel'), (req, res) => {
    ...
    xlsxFile(`./images/${req.file.filename}`).then((rows) => {
        ...
        const objs = rows.map((row) => { // Map the rest of the rows into objects
            ...
            db.query(`insert into sellers (name , phone , city , email , adress , phone2 ) values ('${obj.Name}' , '${obj.Phone}' , '${obj.City}' , '${obj.Email}' , '${obj.Adress}' , '${obj.phone2}' )`, (err, result) => {
                db.query(`insert into action ( place , who , notes) values ( '${req.session.user.root}' , '${req.session.user.id}' , '${req.session.user.name}  Added Seller a Name  ${obj.Name}')`, (err0, result0) => {
                    if (r === 0) {
                        res.json({ check: true });
                    }
                    r++;
