...
route.post('/', check, (req, res) => {
    ...
    if (req.body.qty != req.body.search) {
        if (req.body.qty < req.body.search || req.body.qty == req.body.search) {
            db.query(`update itembooked set qty ='${req.body.qty}' , priceSeller = '${req.body.priceSeller}' , total = '${req.body.priceSeller * req.body.qty}' where id = '${req.body.id}'`,
                (err, result) => {
        ...
        db.query(`update itembooked set priceSeller = '${req.body.priceSeller}' , total = '${req.body.priceSeller * req.body.qty}' where id = '${req.body.id}'`,
            (err, result) => {
                db.query(`insert into action ( place , who , notes) values ( '${req.session.user.root}' , '${req.session.user.id}' , '${req.session.user.name}  Update Quotation code is  ${req.body.id}')`, (err, result) => {
                    res.send(true);
                })
