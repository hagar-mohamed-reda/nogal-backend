...
route.post('/', check, upload.single('excel'), (req, res) => {
    ...
    xlsxFile(`./images/${req.file.filename}`).then((rows) => {
        ...
        const objs = rows.map((row) => { // Map the rest of the rows into objects
            ...
            db.query(`insert into clients (name , phone , city , email , adress , phone2 , area , type ) values ('${obj.Name}' , '${obj.Phone}' , '${obj.City}' , '${obj.email}' , '${obj.Address}' , '${obj.phone2}' , '${obj.area}' , '${obj.type}' )`, (err, result) => {
                db.query(`insert into action ( place , who , notes) values ( '${req.session.user.root}' , '${req.session.user.id}' , '${req.session.user.name}  Added Customer a Name  ${obj.Name}')`, (err0, result0) => {
                    if (r === 0) {
                        res.json({ check: true });
                    }
                    r++;
